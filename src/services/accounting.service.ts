...
export interface AccountingTransactionLineInput {
  accountCode: string;
  debit: number;
  credit: number;
  description?: string;
...
export interface AccountingTransactionInput {
  ...
  referenceType?: 'ORDER' | 'INVOICE' | 'EXPENSE' | 'INCOME' | 'PAYMENT' | 'MANUAL';
  referenceId?: string;
  lines: AccountingTransactionLineInput[];
}

export interface ExpenseInput {
  category: string; // Kira, Elektrik, Reklam, Kargo, Personel, Bakım vb.
  amount: number;
  taxRate?: number; // Örn: 20
  currency?: string;
  paymentMethod: 'CASH' | 'BANK_TRANSFER' | 'CREDIT_CARD';
  accountCode?: string; // 100.01 veya 102.01
  supplierName?: string;
  description: string;
  invoiceNumber?: string;
  isRecurring?: boolean;
  status?: 'CONFIRMED' | 'DRAFT_PENDING_APPROVAL';
  date?: string;
  performedBy?: string;
}
...
export interface PaymentInput {
  ...
  amount: number;
  currency?: string;
  paymentMethod: 'CASH' | 'BANK_TRANSFER' | 'CREDIT_CARD' | 'POS';
  accountCode?: string;
  referenceNo?: string;
  date?: string;
  performedBy?: string;
...
export class AccountingService {
   ...
   */
  private static generateNumber(prefix: string): string {
   ...
   */
  public static addAuditLog(action: string, entityType: string, entityId: string, performedBy: string, details: any): void {
    try {
   ...
   */
  public static postDoubleEntryTransaction(input: AccountingTransactionInput, performedBy: string = 'SYSTEM'): { success: boolean; transactionNumber?: string; error?: string } {
    ...
    try {
      const runTransaction = db.transaction(() => {
        ...
        const trxId = result.lastInsertRowid as number;
   ...
   */
  public static recordOrderFinancials(orderId: string, performedBy: string = 'SYSTEM'): { success: boolean; message?: string } {
    try {
      // 1. Idempotency Kontrolü: Bu sipariş zaten muhasebeleştirildi mi?
      const existing = db.prepare(`SELECT id FROM accounting_transactions WHERE reference_type = 'ORDER' AND reference_id = ?`).get(orderId);
      if (existing) {
      ...
      const prod = db.prepare(`SELECT price, cost_price FROM products WHERE product_code = ?`).get(order.product_code) as any;
      ...
      const lines: AccountingTransactionLineInput[] = [
   ...
   */
  public static addExpense(input: ExpenseInput): { success: boolean; expenseNumber?: string; error?: string } {
    try {
   ...
   */
  public static addIncome(input: IncomeInput): { success: boolean; incomeNumber?: string; error?: string } {
    try {
   ...
   */
  public static confirmDraft(type: 'EXPENSE' | 'INCOME', idOrNumber: string, performedBy: string = 'USER'): { success: boolean; error?: string } {
    try {
      if (type === 'EXPENSE') {
        const expense = db.prepare(`SELECT * FROM expenses WHERE id = ? OR expense_number = ?`).get(idOrNumber, idOrNumber) as any;
        if (!expense) return { success: false, error: 'Gider taslağı bulunamadı.' };
        if (expense.status === 'CONFIRMED') return { success: true };

        db.prepare(`UPDATE expenses SET status = 'CONFIRMED' WHERE id = ?`).run(expense.id);
        ...
        const income = db.prepare(`SELECT * FROM income_entries WHERE id = ? OR income_number = ?`).get(idOrNumber, idOrNumber) as any;
        if (!income) return { success: false, error: 'Gelir taslağı bulunamadı.' };
        if (income.status === 'CONFIRMED') return { success: true };

        db.prepare(`UPDATE income_entries SET status = 'CONFIRMED' WHERE id = ?`).run(income.id);
   ...
   */
  public static createInvoice(input: InvoiceInput): { success: boolean; invoiceNumber?: string; invoiceId?: number; error?: string } {
    ...
    try {
      ...
      let invoiceId = 0;
      const runInvTransaction = db.transaction(() => {
        const stmt = db.prepare(`
        ...
        invoiceId = res.lastInsertRowid as number;
   ...
   */
  public static recordPayment(input: PaymentInput): { success: boolean; paymentNumber?: string; error?: string } {
    try {
   ...
   */
  public static getCashAndBankSummary(): { cashTotal: number; bankTotal: number; liquidAssetsTotal: number; accounts: any[] } {
    try {
      const accounts = db.prepare(`SELECT code, name, type, balance, currency FROM accounting_accounts WHERE code LIKE '100%' OR code LIKE '102%' ORDER BY code ASC`).all() as any[];
   ...
   */
  public static getFinancialSummary(period: 'this_month' | 'last_month' | 'this_year' | 'all' = 'this_month'): any {
    try {
      ...
      const recRow = db.prepare(`SELECT SUM(total_amount - paid_amount) as totalRec FROM invoices WHERE type = 'SALE' AND status != 'PAID' AND status != 'CANCELLED'`).get() as any;
      const payRow = db.prepare(`SELECT SUM(total_amount - paid_amount) as totalPay FROM invoices WHERE type = 'PURCHASE' AND status != 'PAID' AND status != 'CANCELLED'`).get() as any;
   ...
   */
  public static getProfitLossReport(startDate?: string, endDate?: string): any {
    try {
      const start = startDate || '2026-01-01';
      const end = endDate || this.getTodayDateString();

      // Satış Hasılatı
      const salesRow = db.prepare(`SELECT SUM(total_price) as sum FROM orders WHERE status != 'DEC' AND DATE(created_at) >= ? AND DATE(created_at) <= ?`).get(start, end) as any;
      ...
      // Faaliyet Giderleri
      const expRow = db.prepare(`SELECT SUM(amount) as sum FROM expenses WHERE status = 'CONFIRMED' AND date >= ? AND date <= ?`).get(start, end) as any;
   ...
   */
  public static getBalanceSheet(): any {
    try {
      const liquid = this.getCashAndBankSummary();
      const recRow = db.prepare(`SELECT SUM(total_amount - paid_amount) as total FROM invoices WHERE type = 'SALE' AND status != 'PAID'`).get() as any;
      const invRow = db.prepare(`SELECT SUM(stock * COALESCE(cost_price, price * 0.5)) as total FROM products`).get() as any;
      const payRow = db.prepare(`SELECT SUM(total_amount - paid_amount) as total FROM invoices WHERE type = 'PURCHASE' AND status != 'PAID'`).get() as any;
   ...
   * Vergi / KDV Özeti
   */
  public static getTaxSummary(): any {
    try {
      const dateFilter = this.getDateFilterClause('this_month');

      const salesTaxRow = db.prepare(`SELECT SUM(total_price - (total_price / 1.20)) as tax FROM orders WHERE status != 'DEC' ${dateFilter.replace('date', 'created_at')}`).get() as any;
      const inputTaxRow = db.prepare(`SELECT SUM(tax_amount) as tax FROM expenses WHERE status = 'CONFIRMED' ${dateFilter}`).get() as any;
   ...
   */
  public static getProductProfitability(): any[] {
    try {
   ...
   */
  public static getExpenses(): any[] {
    try {
      return db.prepare(`SELECT * FROM expenses ORDER BY id DESC LIMIT 100`).all();
    } catch (e: any) {
      return [];
    }
  }
   ...
   */
  public static getIncomeEntries(): any[] {
    try {
      return db.prepare(`SELECT * FROM income_entries ORDER BY id DESC LIMIT 100`).all();
    } catch (e: any) {
      return [];
    }
  }
   ...
   */
  public static getInvoices(): any[] {
    try {
      return db.prepare(`SELECT * FROM invoices ORDER BY id DESC LIMIT 100`).all();
    } catch (e: any) {
      return [];
    }
  }
  ...
  public static getTransactions(): any[] {
    try {
      const trxs = db.prepare(`SELECT * FROM accounting_transactions ORDER BY id DESC LIMIT 50`).all() as any[];
      const lineStmt = db.prepare(`SELECT * FROM accounting_transaction_lines WHERE transaction_id = ?`);
      return trxs.map(t => ({
        ...t,
   ...
   */
  private static getDateFilterClause(period: string): string {
    const today = new Date();
