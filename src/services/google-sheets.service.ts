...
export class GoogleSheetsService {
   ...
   */
  public static parseSmartStockRows(rows: any[][]): Array<{
    shortCode: string;
    productCode: string;
    name: string;
    color: string;
    ...
    const hasHeaderRow = normalizedHeaders.some(h => 
      codeAliases.includes(h) || nameAliases.includes(h) || stockAliases.includes(h) || priceAliases.includes(h)
    );

    if (hasHeaderRow) {
      startRowIdx = 1;
      normalizedHeaders.forEach((h, idx) => {
        if (codeAliases.includes(h)) codeIdx = idx;
        else if (nameAliases.includes(h)) nameIdx = idx;
        else if (colorAliases.includes(h)) colorIdx = idx;
        else if (sizeAliases.includes(h)) sizeIdx = idx;
        else if (stockAliases.includes(h)) stockIdx = idx;
        else if (priceAliases.includes(h)) priceIdx = idx;
        else if (categoryAliases.includes(h)) categoryIdx = idx;
      });
    }
